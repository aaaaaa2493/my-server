...
stockfish.onmessage = function(event) {
    ...
    if(data[0] === 'info' && data[3] === 'seldepth' && !game_over){

        var evaluation = data[9]/100 * (side_to_move === 'white'? 1: -1)

        if(data[8] === 'cp'){
            document.getElementById('result').innerHTML = 'Оценка позиции: <br>' + evaluation + (data[9] % 100 === 0? '.0': '') + (data[9] % 10 === 0? '0': '');
        }
        else{
            document.getElementById('result').innerHTML = 'Оценка позиции: <br>#' + (data[9] * (side_to_move === 'white'? 1: -1));
        }
...
stockfish.postMessage('go depth ' + default_depth);

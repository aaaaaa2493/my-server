...
var last_moved_being = 'computer';
var last_cursor_hovered = undefined;
var possibles;
var candidate_move;
var black_king = 'e8';
var white_king = 'e1';
    ...
    if(data[0] === 'bestmove' && !dont_touch){
        ...
        for(var i in moves){
            ...
            if(move.to === to){
                ...
                last_moved_being = 'computer';
                break;
            }
        }

        clk(last_cursor_hovered);

    }
    if(data[0] === 'info' && data[3] === 'seldepth' && !game_over && show_evaluation){

        var evaluation = data[9]/100 * (side_to_move === 'white'? 1: -1)

        if(document.getElementById('first_computer').style.display === 'none'){
            document.getElementById('result').innerHTML = 'Оценка позиции: <br>';
        }
        else{
            document.getElementById('result').innerHTML = 'Оценка - ';
        }
...
if(default_depth >= 0){
    stockfish.postMessage("setoption name Skill Level value " + default_depth);
    sdefault_depth = (default_depth <= 10? default_depth: 10 + (default_depth-10)/2|0);
}

function create_UCI_moves(){
...
function unmake_move(){
    ...
    table_history.pop();
    side_to_move = side_to_move === 'black'? 'white': 'black';
    last_moved_being = last_moved_being === 'human'? 'computer': 'human';
...
function additional_check(){
    ...
    if(state === 'Nothing' && !show_evaluation){
        document.getElementById('result').innerHTML = '';
    }
    else if(state === 'Checkmate to white'){
...
function clk(pos){
    ...
    if(dont_touch)return;

    if(!pos)return;

    if(pos.path){
            
        if(pos.path[1].id !== 'table' && pos.path[2].id !== 'table'){
            for(var curr_pos in table){
                document.getElementById(curr_pos).classList.remove('chosen');
                document.getElementById(curr_pos).classList.remove('candidate_move');
                document.getElementById(curr_pos).classList.remove('candidate_capture');
            }
            last_cursor_hovered = undefined;
        }
        else{
            if(pos.path[1].id === 'table'){
                last_cursor_hovered = pos.path[0].id;
            }
            else{
                last_cursor_hovered = pos.path[1].id;
            }
        }
    ...
    if(last_moved_being === 'computer'){
        
        if(promotion_mode || game_over) return;
        ...
        if(pos === 'nothing') return;

        if(pos !== 'nothing' && table[pos].side === side_to_move){

            if(table[pos].side === side_to_move){
                document.getElementById(pos).classList.add('chosen');
            }

            var candidate_move, index;
            possibles = get_moves(side_to_move)[pos];

            for(index in possibles){
                candidate_move = possibles[index];
                document.getElementById(candidate_move.to).classList.add('candidate_'+(candidate_move.capture? 'capture': 'move'));
            }
...
function drop(ev) {
    ...
    var target = ev.path.length === 7 || ev.path.length === 9? ev.target: ev.path[1];
    var pos = target.id;
    last_cursor_hovered = pos;
    var index;
    
    if(target.classList.contains('candidate_move') || target.classList.contains('candidate_capture')){
        
        for(index in possibles){
            candidate_move = possibles[index];
            
            if(candidate_move.to === pos){
                
                if(candidate_move.target === 'pawn' && candidate_move.promotion && !promotion_mode){
                    move_graphic(candidate_move);
                    break;
                }
                ...
                last_moved_being = 'human';
                document.getElementById('first_computer').style.display = 'none';

                if(default_depth >= 0){
                    stockfish.postMessage(create_UCI_moves());
                    stockfish.postMessage("go depth " + (default_depth+ (Math.random() * 5 | 0) - 2) );
                }
                else{
                    make_random_move();
                }
...
function choose_new_figure(type){
    
    candidate_move.new_figure = type;
    make_move(candidate_move);
    move_graphic(candidate_move);
...
function undo_button(){
    ...
    if(!promotion_mode && !dont_touch){
        if(last_moved_being === 'human'){
            unmake_move_graphic(unmake_move());
        }
        else if(moves_history.length === 1){
            unmake_move_graphic(unmake_move());
        }
        else if(moves_history.length > 1){
            unmake_move_graphic(unmake_move());
        ...
        if(moves_history.length === 0){
            document.getElementById('first_computer').style.display = 'block';
        }
...
function reverse_board(){
    change_color_click(board_position === 'standart'? 'reversed': 'standart');
    change_color_out(board_position === 'standart'? 'reversed': 'standart');
}
...
function include_engine_in(from_who){
    if(from_who === true && show_evaluation === true){
        document.getElementById('include_engine').style.display = 'none';
        document.getElementById('exclude_engine').style.display = 'block';
    }
    else if(from_who === false && show_evaluation === false){
        document.getElementById('exclude_engine').style.display = 'none';
        document.getElementById('include_engine').style.display = 'block';
    }
}

function include_engine_out(from_who){
    if(from_who === false && show_evaluation === true){
        document.getElementById('include_engine').style.display = 'block';
        document.getElementById('exclude_engine').style.display = 'none';
    }
    else if(from_who === true && show_evaluation === false){
        document.getElementById('exclude_engine').style.display = 'block';
        document.getElementById('include_engine').style.display = 'none';
    }
}

function include_engine_click(from_who){
    if(!dont_touch){
        if(from_who === true){
